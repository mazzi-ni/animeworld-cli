...
def play_id(ep_index):
    while True:
        ...
        try:
            url = anime.original_video_url(anime.BASE_API_URL + anime.anime_data["eps_id"][ep_index])
            player.play(url)
...
def download(
    ...
    ep_range = q.text(
        "Select range of Episode to download: [1-"
        + str(anime.anime_data["eps_n"])
        + "]",
